...
# SETTINGS
env.project = 'futurice_checklist'
...
def deploy():
    ...
    #manage('migrate common --fake 0001 --noinput')
...
def manage(args):
    with virtualenv():
        with cd(env.project_root):
            return sudo('DJANGO_SETTINGS_MODULE=futurice_checklist.settings python manage.py {args}'.format(args=args))
...
def prepare_node_packages():
    with virtualenv():
        ...
        with cd(env.basepath):
            sudo('[ -f "venv/bin/node" ] || nodeenv -p --node=0.10.33')
